...
const COLORS = {
  ...
  bogo: "#8e24aa", // Purple color for BOGO badge
};
...
const HomeKitchenDetails = ({ route }) => {
  ...
  // Fetch kitchen details - Updated with session check
  const fetchKitchenDetails = useCallback(async () => {
    try {
      setLoading(true);
      if (!isSessionInitialized) {
        console.log("Session not initialized, waiting...");
  ...
  }, [route.params?.kitchenId, isSessionInitialized, initializeSession]);

  // Fetch cart data - Updated with proper session handling
  const fetchCartData = useCallback(async () => {
    try {
      if (!isSessionInitialized) {
        console.log("Session not initialized for cart fetch");
        return;
      }
      ...
      console.log("Fetching cart with payload:", payload);
      ...
      // Don't show error message if it's just a session issue
  ...
  }, [sessionId, isSessionInitialized, fetchUserData, savePastKitchenDetails, clearPastKitchenDetails]);

  // Refresh all data - Updated with session initialization
  const refreshData = useCallback(async () => {
    if (!isSessionInitialized) {
      console.log("Initializing session before refresh...");
      await initializeSession();
    }
  ...
  }, [isSessionInitialized, initializeSession, fetchKitchenDetails, fetchCartData, fetchPastKitchenDetails]);

  // Initial data load
  useEffect(() => {
    if (route.params?.kitchenId && isSessionInitialized) {
      console.log("Loading data with session:", sessionId);
      refreshData();
    }
  }, [route.params?.kitchenId, isSessionInitialized, sessionId]);
    ...
    return categories.map((category) => {
      ...
      return {
        ...
        items: categoryItems.map((item) => ({
          ...
          buy_one_get_one_free: item.buy_one_get_one_free, // Convert string to boolean
  ...
  // Update cart item quantity with proper session handling
  const updateCartItem = useCallback(
    async (itemId, action, force = false) => {
      ...
      try {
        ...
        showMessage({
          message: action === "add" ? "Item added to cart" : "Item removed from cart",
          type: "success",
        });
      } catch (error) {
  ...
  const handleViewCart = useCallback(() => {
    ...
    navigation.navigate("CartScreen", {
      ...
      userId: user?.id,
      sessionId: sessionId, // Pass session ID to cart screen
    });
  }, [navigation, kitchenData, cartItems, pastKitchenDetails, user?.id, sessionId]);
        ...
        <View style={styles.kitchenDetails__menuItemContent}>
          ...
          <View style={styles.kitchenDetails__priceDiscountContainer}>
            ...
            {item.buy_one_get_one_free && (
              <View style={styles.kitchenDetails__bogoBadge}>
                <Icon name="gift" size={10} color={COLORS.text.light} style={styles.kitchenDetails__bogoIcon} />
                <Text style={styles.kitchenDetails__bogoText}>BOGO</Text>
              </View>
            )}
          </View>
  ...
  // Show loading if session is not initialized
  if (!isSessionInitialized && loading) {
    return (
                ...
                <View style={styles.modal__scrollContent}>
                  ...
                  >
                    <View style={styles.modal__body}>
                      ...
                      {/* BOGO Badge in Modal */}
                      {selectedItem.buy_one_get_one_free && (
                        <View style={styles.modal__bogoBadge}>
                          <Icon name="gift" size={16} color={COLORS.text.light} />
                          <Text style={styles.modal__bogoText}>
...
const styles = StyleSheet.create({
  ...
  // BOGO Badge Styles
  kitchenDetails__bogoBadge: {
    backgroundColor: COLORS.bogo,
    flexDirection: "row",
    alignItems: "center",
    paddingHorizontal: 6,
    paddingVertical: 2,
  ...
  kitchenDetails__bogoIcon: {
    marginRight: 2,
  },
  kitchenDetails__bogoText: {
    color: COLORS.text.light,
    fontSize: FONT.XS,
    fontWeight: "bold",
  },
