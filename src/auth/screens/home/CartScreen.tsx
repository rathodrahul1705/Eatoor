...
// New type for payment method from API
...
type CartApiResponse = {
  ...
  order_count: number;
  payment_method_details: PaymentMethodDetail[]; // Added this field
};
...
// Payment method options - Updated to map API codes to UI options
type PaymentMethodOption = {
  ...
  code: number; // API code for payment method
  disabled?: boolean;
  disabledReason?: string;
  isActive: boolean;
};

// Helper function to map API payment methods to UI options
const mapPaymentMethodToOption = (apiMethod: PaymentMethodDetail): PaymentMethodOption | null => {
  const { id, name, code, is_active } = apiMethod;
  
  // Map based on code or name
  switch (code) {
    ...
    default:
      // Try to infer from name if code doesn't match
      const nameLower = name.toLowerCase();
      if (nameLower.includes('cod') || nameLower.includes('cash') || nameLower.includes('delivery')) {
        return {
          id,
          title: name,
          subtitle: 'UPI / Cash',
      ...
      } else if (nameLower.includes('online') || nameLower.includes('card') || nameLower.includes('upi')) {
        return {
          id,
          title: name,
          subtitle: 'UPI / Cards / Net Banking',
      ...
      } else if (nameLower.includes('wallet') || nameLower.includes('eatoor money')) {
        return {
          id,
          title: name,
          subtitle: 'Use your wallet balance',
...
// Default payment methods (fallback if API doesn't provide)
...
const CartScreen = ({ route, navigation }) => {
  ...
  // Payment method selection states - Updated to use dynamic options
  const [paymentMethodOptions, setPaymentMethodOptions] = useState<PaymentMethodOption[]>(DEFAULT_PAYMENT_METHOD_OPTIONS);
  ...
  const paymentMethodSlideAnimationRef = useRef<Animated.CompositeAnimation | null>(null);
  const modalDragAnimationRef = useRef<Animated.CompositeAnimation | null>(null);
    ...
    PanResponder.create({
      ...
      onPanResponderRelease: (_, gestureState) => {
        ...
        if (gestureState.dy > 100 || gestureState.vy > 0.5) {
          ...
          if (modalDragAnimationRef.current) {
            modalDragAnimationRef.current.stop();
          }
          modalDragAnimationRef.current = Animated.spring(modalDragOffset, {
            toValue: 0,
            useNativeDriver: true,
            tension: 50,
            friction: 7,
          });
          modalDragAnimationRef.current.start(() => {
            modalDragAnimationRef.current = null;
          });
        }
      },
    })
  ...
  useEffect(() => {
    return () => {
      ...
      if (paymentMethodSlideAnimationRef.current) {
        paymentMethodSlideAnimationRef.current.stop();
      }
      if (modalDragAnimationRef.current) {
        modalDragAnimationRef.current.stop();
      }
  ...
  // Initialize session ID - improved logic
  ...
  useEffect(() => {
    ...
    if (useWallet && walletBalanceAmount >= totalAmount && finalAmount === 0) {
      ...
      const walletOption = paymentMethodOptions.find(option => option.type === 'wallet');
    ...
    if (!selectedPaymentMethod) {
      // First try to find a default active payment method
      const defaultOption = paymentMethodOptions.find(option => option.isActive);
      if (defaultOption) {
        setSelectedPaymentMethod(defaultOption);
      } else {
        // Fallback to any payment method
        const onlineOption = paymentMethodOptions.find(option => option.type === 'online');
        if (onlineOption) {
          setSelectedPaymentMethod(onlineOption);
        }
      }
    }
  }, [useWallet, walletBalance, cartData, paymentMethodOptions]);

  // Handle payment status animations with proper cleanup
  useEffect(() => {
  ...
  // Handle modal slide animation with proper cleanup
  useEffect(() => {
    if (showPaymentModal) {
      if (slideAnimationRef.current) {
        slideAnimationRef.current.stop();
      ...
      slideAnimationRef.current = Animated.timing(slideAnim, {
        toValue: 0,
        duration: 300,
  ...
  const openPaymentMethodModal = () => {
    ...
    paymentMethodSlideAnimationRef.current = Animated.timing(paymentMethodSlideAnim, {
      toValue: 0,
      duration: 350,
      easing: Easing.bezier(0.25, 0.46, 0.45, 0.94),
      useNativeDriver: true,
    });
    paymentMethodSlideAnimationRef.current.start(() => {
      paymentMethodSlideAnimationRef.current = null;
    });
  };
  ...
  const fetchCartData = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (updatedResponse.payment_method_details) {
          const mappedOptions = updatedResponse.payment_method_details
            .map(mapPaymentMethodToOption)
            .filter((option): option is PaymentMethodOption => option !== null)
            .filter(option => option.isActive); // Only show active payment methods
            
          if (mappedOptions.length > 0) {
            setPaymentMethodOptions(mappedOptions);
            
            // Auto-select the first active payment method if none selected
            if (!selectedPaymentMethod) {
              const defaultOption = mappedOptions.find(option => option.isActive);
              if (defaultOption) {
                setSelectedPaymentMethod(defaultOption);
              }
            } else {
              // Check if currently selected method is still available
              const currentMethodStillAvailable = mappedOptions.some(
                option => option.code === selectedPaymentMethod.code
              );
              if (!currentMethodStillAvailable) {
                // Select the first available method
                const newDefault = mappedOptions.find(option => option.isActive);
                if (newDefault) {
            ...
            // If no payment methods from API, use defaults
            setPaymentMethodOptions(DEFAULT_PAYMENT_METHOD_OPTIONS.filter(option => option.isActive));
          }
        } else {
          // If no payment_method_details in response, use defaults
          setPaymentMethodOptions(DEFAULT_PAYMENT_METHOD_OPTIONS.filter(option => option.isActive));
        }
      } else {
  ...
  const renderPaymentMethodModal = () => {
    ...
    const activePaymentMethods = paymentMethodOptions.filter(option => option.isActive);
          ...
          <TouchableOpacity 
            ...
            onPress={() => {
              ...
              if (!newUseWallet && finalAmount === 0) {
                const defaultOption = paymentMethodOptions.find(option => option.isActive && option.type !== 'wallet');
                if (defaultOption) {
                  setSelectedPaymentMethod(defaultOption);
                }
              }
              ...
              if (newUseWallet && balance >= totalAmount) {
                const walletOption = paymentMethodOptions.find(option => option.type === 'wallet' && option.isActive);
                if (walletOption) {
                  setSelectedPaymentMethod(walletOption);
                }
              }
    ...
    return (
      <View style={styles.paymentMethodInlineContainer}>
        ...
        <TouchableOpacity 
          ...
          disabled={showBlurOverlay || isPaymentInProgress || paymentMethodOptions.length === 0}
          activeOpacity={0.7}
        >
          <View style={styles.paymentMethodContent}>
            ...
            <View style={styles.textContainer}>
              ...
              <Text style={styles.paymentDetails} numberOfLines={1}>
                {finalAmount === 0 ? 'Paid with Eatoor Money' : selectedPaymentMethod?.subtitle || 'Select payment method'}
              </Text>
            </View>
          </View>
        </TouchableOpacity>
        ...
        <TouchableOpacity 
          style={[
            ...
            (isPaymentInProgress || paymentStatus === 'processing' || paymentMethodOptions.length === 0) && styles.proceedButtonDisabled,
            !isAddressSelected && styles.selectLocationButton
          ]}
          onPress={initiatePayment}
          disabled={paymentStatus === 'processing' || paymentVerification.verifying || showBlurOverlay || isPaymentInProgress || paymentMethodOptions.length === 0}
          activeOpacity={0.8}
        >
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {isCartEmpty ? (
        ...
        <>
          {/* Updated Header with better iOS/Android compatibility */}
...
const styles = StyleSheet.create({
  ...
  // Updated Header Styles - Better iOS/Android compatibility
  header: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  errorContent: {
    ...
    alignItems: 'center',
    padding: scale(20),
    paddingTop: Platform.OS === 'ios' ? verticalScale(40) : verticalScale(60),
  },
  errorIcon: {
    marginBottom: verticalScale(12),
  },
